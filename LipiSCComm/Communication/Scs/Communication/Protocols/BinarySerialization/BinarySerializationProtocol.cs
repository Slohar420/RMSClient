...
namespace Lipi.Communication.Scs.Communication.Protocols.BinarySerialization
{
    /// <summary>
    ...
    public class BinarySerializationProtocol : IScsWireProtocol
    {
        ...
        public byte[] GetBytes(IScsMessage message)
        {
            ...
            if (message.GetType().Name == "ScsMessage")
                serializedMessage = Encoding.Default.GetBytes(message.ToString());    //Serialize the message to a byte array
            else
        ...
        protected virtual IScsMessage DeserializeMessage(byte[] bytes)
        {
            ...
            using (var deserializeMemoryStream = new MemoryStream(bytes))
            {
                ...
                return (deserializeMemoryStream as IScsMessage);
            }
        }
